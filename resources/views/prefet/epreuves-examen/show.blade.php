...
<div class="card" style="max-width:640px;">
    <div class="flex justify-between items-start mb-4">
        ...
        <x-retour-button fallback-route="prefet.epreuves-examen.index" label="← Retour" />
    </div>
    ...
    <div class="mt-4">
        <form method="POST" action="{{ route('prefet.epreuves-examen.destroy', $epreuveExamen) }}"
              data-confirm-delete
              data-confirm-message="Supprimer définitivement l'épreuve « {{ $epreuveExamen->titre }} » ?">
            @csrf @method('DELETE')
            <input type="hidden" name="redirect_to" value="{{ route('prefet.epreuves-examen.index') }}">
