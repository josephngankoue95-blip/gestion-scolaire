...
<div class="card" style="max-width:640px;">
    ...
    @php
        $fichierOk = $epreuveComposition->fichier && \Illuminate\Support\Facades\Storage::disk('public')->exists($epreuveComposition->fichier);
        $corrigeOk = $epreuveComposition->fichier_corrige && \Illuminate\Support\Facades\Storage::disk('public')->exists($epreuveComposition->fichier_corrige);
    @endphp
    ...
    <div class="flex gap-3 mt-6">
        ...
        <a href="{{ asset('storage/'.$epreuveComposition->fichier_corrige) }}" target="_blank" class="btn-outline w-full">
            <i data-lucide="check-circle" class="w-4 h-4"></i> Corrigé
        </a>
        @endif
    </div>
    ...
    <div class="mt-4">
        <form method="POST" action="{{ route('teacher.epreuves-composition.destroy', $epreuveComposition) }}"
            ...
            <button type="button" class="text-red-500 text-sm" onclick="this.closest('form').requestSubmit()">
                <i data-lucide="trash-2" class="w-4 h-4 inline"></i> Supprimer
            </button>
        </form>
    </div>
</div>
