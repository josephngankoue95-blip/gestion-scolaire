...
<div class="container">
    ...
    <div class="topbar flex items-center justify-between gap-3 flex-wrap">
        <div class="title">Dossier scolarité</div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-4">
        <div class="lg:col-span-2 flex flex-col gap-4">
            ...
            <div class="card">
                <div class="flex justify-between items-center mb-4">
                    <div class="text-center">
                        ...
                        <p class="text-sm text-gray-500">
                            {{ $scolarite->classe->nom }} — {{ $scolarite->anneeScolaire->libelle }}
                        </p>
                    </div>
                    <span class="{{ $scolarite->solde() <= 0 ? 'badge badge-green' : 'badge badge-red' }} text-sm">
                ...
                <div class="table-wrapper">
                    <table class="table-base">
                        <thead>
                            <tr>
                                <th>Rubrique</th><th>Dû</th><th>Payé</th><th>Solde</th><th>Statut</th>
                            </tr>
                        </thead>
                        <tbody>
                            @php
                            ...
                            @foreach ($rubriques as $r)
                                @if($r['du'] > 0)
                                    @php $s = $scolarite->statutTranche($r['type']); $reste = $r['du'] - $r['paye']; @endphp
                                    <tr>
            ...
            <div class="card">
                <h4 class="font-semibold text-gray-800 mb-3 text-center">Historique des paiements</h4>
        ...
        <div class="card mx-auto w-full">
            <h4 class="font-semibold text-gray-800 mb-4 text-center">Enregistrer un paiement</h4>
            <form method="POST" action="{{ route('admin.scolarite.paiements.store', $scolarite) }}" class="space-y-3">
                @csrf
                <div class="form-group text-center">
                    <label class="form-label block text-center">Rubrique *</label>
                    <select name="type" required class="form-select text-center">
                ...
                <div class="form-group text-center">
                    <label class="form-label block text-center">Montant (FCFA) *</label>
                    <input type="number" name="montant" required min="1" class="form-input text-center">
                    @error('montant') <p class="form-error">{{ $message }}</p> @enderror
                </div>
                <div class="form-group text-center">
                    <label class="form-label block text-center">Date *</label>
                    <input type="date" name="date_paiement" required class="form-input text-center" value="{{ date('Y-m-d') }}">
                </div>
                <div class="form-group text-center">
                    <label class="form-label block text-center">Note</label>
