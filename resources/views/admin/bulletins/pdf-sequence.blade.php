...
<div class="titre-bulletin">
    {{ $isEn ? 'SEQUENTIAL REPORT' : 'BULLETIN SÉQUENTIEL' }}
    / SEQUENTIAL REPORT —
    {{ strtoupper($periode->nom) }}
    ({{ strtoupper($periode->trimestre->nom) }})
</div>
...
<thead>
<tr>
    <th style="width:30%; text-align:left; padding-left:4px;">
        {{ $isEn ? 'Subjects' : 'Matières' }}
    </th>
    <th style="width:10%;">{{ $isEn ? 'Mark/20' : 'Note/20' }}</th>
    <th style="width:7%;">{{ $isEn ? 'Coef.' : 'Coef.' }}</th>
    <th style="width:10%;">{{ $isEn ? 'Mark×Coef' : 'Note×Coef' }}</th>
    <th style="width:8%;">{{ $isEn ? 'Rank' : 'Rang' }}</th>
    <th style="width:10%;">{{ $isEn ? 'Class Avg' : 'Moy Classe' }}</th>
    <th style="width:10%;">{{ $isEn ? 'Marks≥10' : 'Notes≥10' }}</th>
    <th style="width:15%;">{{ $isEn ? 'Remarks' : 'Appréciations' }}</th>
</tr>
</thead>
<tbody>
...
@foreach ($bulletin['details'] as $detail)
    ...
    @php
        ...
        $noteClass = ($note !== null && $note < 10) ? 'note-inf' : 'note-sup';
...
<tr>
    ...
    <td style="width:27%; vertical-align:top;">
        ...
        <table style="width:100%;border-collapse:collapse;margin-top:2px;">
            ...
            <tr>
                <td class="resultat-label" style="border:1px solid #ccc;padding:2px 3px;font-size:7px;">
                    {{ $isEn ? 'Encouragements' : 'Encouragements' }}
                </td>
                <td style="border:1px solid #ccc;padding:2px 3px;">{{ $isEn ? 'No' : 'Non' }}</td>
            </tr>
            <tr>
    ...
    <td style="width:23%; vertical-align:top;">
        ...
        <div style="border:1px solid #ccc;padding:5px;font-size:7.5px;margin-top:2px;min-height:56px;">
            @php
                $matieres10 = collect($bulletin['details'])->filter(fn($d) => ($d['note'] ?? 0) >= 10)->count();
...
<tr>
    ...
    <td>
        ...
        <div style="font-size:7px;">
            {{ $isEn
                ? 'THE PRINCIPAL'
                : 'LE '.strtoupper($etablissement->type_etablissement ?? 'PROVISEUR') }}
        </div>
