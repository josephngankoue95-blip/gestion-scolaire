    ...
    <div style="border:1px solid #e5e7eb;border-radius:8px;padding:14px;margin-bottom:10px;">
        <div class="flex justify-between items-start mb-2">
            <div>
                ...
                <p class="text-xs text-gray-500">{{ $r->eleve->nomComplet() }} — {{ $r->created_at->format('d/m/Y H:i') }}
                    <span class="badge badge-blue ml-1" style="font-size:10px;">{{ str_replace('_',' ',$r->type) }}</span>
                </p>
            </div>
            <span class="badge badge-{{ $r->statut==='traitee'?'green':($r->statut==='rejetee'?'red':'amber') }}">{{ ucfirst(str_replace('_',' ',$r->statut)) }}</span>
        </div>
        <p class="text-sm text-gray-600 mb-3">{{ $r->message }}</p>

        @if(in_array($r->statut, ['en_attente','en_cours']))
